      ...
      if (isClosing) {
        ...
        if (tagName === "li") {
          ...
          tokens.push({
            ...
            style: {
              ...
              spoiler: false,
              size: sizeStack[sizeStack.length - 1] || 14,
        ...
        if (tagName === "font") {
          ...
          colorStack.push(
            colorMatch
              ? colorMatch[1]
              : colorStack[colorStack.length - 1] || null,
          );
        }
        if (tagName === "span") {
          ...
          if (colorMatch) colorToPush = colorMatch[1];
            ...
            if (
              cls.includes("hljs-keyword") ||
              cls.includes("hljs-built_in") ||
              cls.includes("hljs-literal") ||
              cls.includes("hljs-type")
            )
              colorToPush = "#ff7b72"; // pink
            else if (cls.includes("hljs-string"))
              colorToPush = "#a5d6ff"; // light blue
            else if (cls.includes("hljs-number"))
              colorToPush = "#79c0ff"; // blue
            else if (cls.includes("hljs-comment"))
              colorToPush = "#8b949e"; // gray
            else if (
              cls.includes("hljs-function") ||
              cls.includes("hljs-title")
            )
              colorToPush = "#d2a8ff"; // purple
            else if (
              cls.includes("hljs-variable") ||
              cls.includes("hljs-params") ||
              cls.includes("hljs-attr") ||
              cls.includes("hljs-property")
            )
              colorToPush = "#ffa657"; // orange
          }
...
export function layoutRichText(
  ...
  for (const seg of segments) {
    ...
    if (seg.type === "image") {
      if (
        currentLine.width + seg.width > effectiveMaxWidth &&
        currentLine.items.length > 0
      ) {
        pushLine();
      ...
      currentLine.items.push({
        type: "image",
        token: seg.token,
        x: currentLine.width + totalIndent,
        w: seg.width,
        h: seg.height,
      });
  ...
  if (inPre) {
    ...
    lines.push({
      width: 0,
      height: 12,
      isBlockquote: currentLine.isBlockquote,
      inPre: true,
      items: [],
    });
...
export function drawRichText(
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!line.inPre && preStartY !== -1) {
      drawCodeBg(
        ctx,
        startX + blockquoteOffset,
        preStartY,
        contentWidth - blockquoteOffset,
        cursorY - preStartY,
        currentPreLang,
      );
      preStartY = -1;
        ...
        if (item.style.spoiler) {
          ...
          if (item.style.underline || item.style.strike) {
            ...
            if (item.style.strike) {
              ...
              ctx.lineTo(
                startX + item.x + item.w,
                textY - item.style.size * 0.3,
              );
            }
            ctx.stroke();
          }
...
function drawCodeBg(
  ...
  ctx.beginPath();
  ctx.arc(x + 32, y + 16, 5, 0, Math.PI * 2);
  ctx.fill();
  ctx.fillStyle = "#27c93f";
  ctx.beginPath();
  ctx.arc(x + 48, y + 16, 5, 0, Math.PI * 2);
  ctx.fill();
