...
function isNegativelyCached(url: string): boolean {
    ...
    if (Date.now() - t > NEGATIVE_CACHE_TTL) { negativeCache.delete(url); return false; }
    return true;
}
function markFailed(url: string) { negativeCache.set(url, Date.now()); }

// Compact a media URL for logging: hide access tokens, keep server/mediaId.
function shortUrl(url: string): string {
    try {
        const u = new URL(url);
        const m = u.pathname.match(/\/media\/(?:download|thumbnail)\/([^/]+)\/([^/?#]+)/);
        if (m) return `${m[1]}/${m[2]}${u.pathname.includes('thumbnail') ? ' (thumb)' : ''}`;
...
export async function downloadFile(url: string, label = 'media', timeoutMs = 15000): Promise<ResolvedMedia | null> {
    const resolvedUrl = url;
    if (isNegativelyCached(resolvedUrl)) {
        log.debug(`media ${label} skipped (negative-cached): ${shortUrl(resolvedUrl)}`);
        return null;
    }
    const started = Date.now();
    try {
        const headers: Record<string, string> = {
            'User-Agent': 'QuotedMatrix/1.0 (+https://github.com/Vadim-Khristenko/quoted-matrix)',
        ...
        if (!res.ok) {
            ...
            log.warn(`${label} ${shortUrl(resolvedUrl)} (${ms}ms)`, { status: res.status });
            return null;
        }
        const contentType = res.headers.get('content-type') || '';
        const ab = await res.arrayBuffer();
        const buffer = Buffer.from(ab);
        log.debug(`media ${label} ← ${res.status} ${(buffer.length/1024).toFixed(1)}KB ${contentType} (${ms}ms) ${shortUrl(resolvedUrl)}`);
        ...
        const animated = contentType.includes('webp') || contentType.includes('gif') || ['webm','mp4','gif','tgs'].includes(ext) || lower.includes('animated');
...
export function normalizeFfmpegPath(raw: unknown): string {
    ...
    p = p.replace(/\f/g, '\\f').replace(/\x08/g, '\\b').replace(/\t/g, '\\t')
         .replace(/\r/g, '\\r').replace(/\v/g, '\\v').replace(/\0/g, '\\0');
    return p || 'ffmpeg';
}
...
async function tryExtractVideoFrame(inputPath: string, outputPath: string, frameNum = 0): Promise<boolean> {
    try {
 ...
 */
export async function extractFramesSharp(inputBuffer: Buffer, outDir: string, maxFrames = 60): Promise<string[]> {
    try {
        ...
        for (let f = 0; f < pages; f += step) {
            ...
            const outPath = path.join(outDir, `frame_${String(outIdx).padStart(3, '0')}.png`);
            writeFileSync(outPath, png);
            files.push(outPath);
            outIdx++;
            if (outIdx >= maxFrames) break;
        }
        log.info(`Extracted ${files.length}/${pages} frames via sharp (alpha preserved) from ${meta.format}`);
        return files;
    } catch (e) {
        log.debug('sharp frame extraction failed, will fall back to ffmpeg', String(e).slice(0, 120));
        return [];
    }
}

export async function extractVideoFrames(inputPath: string, outDir: string, maxFrames = 60, fps = 20): Promise<string[]> {
    try {
        const { $ } = await import('bun');
        const ff = await getFfmpegPath();
        mkdirSync(outDir, { recursive: true });
        await $`${ff} -y -loglevel error -i ${inputPath} -vf fps=${fps} -vframes ${maxFrames} ${path.join(outDir, 'frame_%03d.png')}`.quiet();
        const files = readdirSync(outDir).filter(f => f.startsWith('frame_') && f.endsWith('.png')).sort().map(f => path.join(outDir, f));
        log.info(`Extracted ${files.length} frames via ffmpeg from ${path.basename(inputPath)}`);
        return files;
    } catch (e) {
...
async function tryRenderTgs(buffer: Buffer, outputPath: string): Promise<boolean> {
    try {
        ...
        JSON.parse(jsonStr);
        log.debug('lottie vector sticker detected, renderer unavailable – using placeholder');
        return false;
    } catch (e) {
...
export async function loadCanvasImage(url: string, opts: { emoji?: boolean } = {}): Promise<Image | null> {
    if (!url) return null;
    ...
    try {
        ...
        if (url.startsWith('http://') || url.startsWith('https://')) {
            ...
            if (!dl && url.includes('/_matrix/client/v1/media/thumbnail/')) {
                const fallbackUrl = url
                    .replace('/_matrix/client/v1/media/thumbnail/', '/_matrix/client/v1/media/download/')
                    .replace(/\?.*$/, '');
                log.debug(`Thumbnail 404, trying download URL for ${logCtx}`, fallbackUrl.slice(0, 100));
        ...
        if (['webp', 'gif', 'avif', 'png', 'jpg', 'jpeg', 'tiff', 'svg'].includes(ext)) {
            try {
                const pngBuffer = await sharp(buffer, { animated: false, limitInputPixels: 8000*8000 }).png().toBuffer();
                if (!animated) writeFileSync(cachePng, pngBuffer);
                const img = await loadImage(pngBuffer);
                log.debug(`Decoded ${logCtx} via sharp`, ext, `${img.width}x${img.height}`);
                return img;
            } catch (e) {
        ...
        if (['webm', 'mp4', 'mov', 'mkv', 'gif', 'webp'].includes(ext) || mime.startsWith('video/')) {
            ...
            if (ok) {
                ...
                log.info(`Extracted first video frame for ${logCtx}`, `${img.width}x${img.height}`);
                return img;
            }
            try {
        ...
        try {
            ...
            log.debug(`could not decode ${logCtx} (${mime || ext || 'unknown'}) ${shortUrl(url)}`);
            return null;
        }

    } catch (e) {
        log.warn(`loadCanvasImage error ${logCtx}`, shortUrl(url), String(e).slice(0, 80));
        return null;
    }
}
...
const PLACEHOLDER_META: Record<PlaceholderKind, { icon: string; label: string }> = {
...
export async function generatePlaceholder(kind: PlaceholderKind = 'sticker'): Promise<Image | null> {
    if (placeholderCache.has(kind)) return placeholderCache.get(kind)!;
    try {
        const { Canvas } = await import('@napi-rs/canvas');
        const { registerFonts, FONT_FAMILY_SANS, FONT_FAMILY_ICONS } = await import('./registerFonts');
        registerFonts();
        const S = 256;
...
export async function loadStickerImage(url: string, isAnimated = false): Promise<Image | null> {
...
export async function loadEmojiImage(url: string, altText?: string): Promise<Image | null> {
    const img = await loadCanvasImage(url, { emoji: true });
    if (img) return img;
    // Fallback: return null – caller will draw alt text via font (Noto Color Emoji)
    if (altText) {
        log.debug('Emoji image failed, will fallback to text rendering', altText, url.slice(0,80));
    }
    return null;
}
