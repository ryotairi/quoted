...
const META: Record<Method, { label: string; code: string; bg: string; threshold: number }> = {
...
function build(method: Method, mod: string, args: any[]): any[] {
    ...
    const statusTag = status !== undefined ? ` ${paint('90', '(STATUS CODE:')} ${paint(m.code, String(status))}${paint('90', ')')}` : '';
